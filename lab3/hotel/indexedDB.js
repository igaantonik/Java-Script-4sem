...
function initDatabase() {
    ...
    request.onsuccess = function () {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            console.log("in");
            updateRoomTileColor('101', 'room1', 'text1');
            updateRoomTileColor('102', 'room2', 'text2');
            updateRoomTileColor('103', 'room3', 'text3');
            updateRoomTileColor('104', 'room4', 'text4');
            updateRoomTileColor('105', 'room5', 'text5');
        });
...
function addReservation(room, requestedArrivalDate, requestedCheckoutDate, guest) {
        ...
        if (cursor) {
            ...
            if (reservation.room === room) {
                ...
                if ((start <= reservationArrivalDate && end >= reservationCheckoutDate) ||
                    ...
                    window.alert('Pokój niedostępny!');
                    return 0;
                }
            }
            cursor.continue();
            ...
            if(available){
                store.add({guest: guest, room: room , arrivalDate: requestedArrivalDate,checkoutDate: requestedCheckoutDate});
                window.alert('Rezerwacja została pomyślnie dokonana!');
                return 1;
            }
        }
    };
...
function isRoomReservedToday(roomNumber) {
    ...
    return new Promise((resolve, reject) => {
        store.openCursor().onsuccess = function(event) {
            ...
            if (cursor) {
                ...
                if (reservation.room === roomNumber && today >= arrivalDate && today <= checkoutDate) {
                    resolve(true);
                    return;
                }
                cursor.continue();
        ...
        transaction.onerror = function(event) {
            reject("Błąd podczas sprawdzania rezerwacji");
        };
    });
}


function updateRoomTileColor(roomNumber,id,textId) {
    const roomCard = document.getElementById(id);
    const availabilityText = document.getElementById(textId);
    console.log(id,roomCard,isRoomReservedToday(roomNumber));
    isRoomReservedToday(roomNumber)
        .then((isReserved) => {
            if (isReserved) {
                console.log("is");
                roomCard.classList.add('reserved');
                availabilityText.innerHTML = '<small class="text-muted">NIEDOSTĘPNY</small>';
            } else {
                console.log("not");
                roomCard.classList.remove('reserved');
                availabilityText.innerHTML = '<small class="text-muted">DOSTĘPNY</small>';
            }
